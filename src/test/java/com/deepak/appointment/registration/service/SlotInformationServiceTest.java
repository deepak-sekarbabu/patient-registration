...
class SlotInformationServiceTest {
    ...
    @Mock
    private SlotInformationRepository slotInformationRepository;
    ...
    void getAvailableDates_shouldReturnListOfDates_whenDatesAvailable() {
        ...
        verify(slotInformationRepository, times(1)).findAvailableDatesByClinicAndDoctor(clinicId, doctorId);
    }
    ...
    void getAvailableSlots_shouldReturnMapOfSlots_whenSlotsAvailable() {
        ...
        when(slotInformationRepository.findAvailableSlotsByClinicDoctorAndDate(clinicId, doctorId, date))
            .thenReturn(Collections.singletonList(slot));

        // Act
        Map<String, List<Map<String, String>>> slotsMap = slotInformationService.getAvailableSlots(clinicId, doctorId, date);
        ...
        // Depending on your SlotInformationService logic, you might also want to assert the "available" status
        // assertEquals("true", slotsMap.get("Morning").get(0).get("available"));
        verify(slotInformationRepository, times(1)).findAvailableSlotsByClinicDoctorAndDate(clinicId, doctorId, date);
    }
