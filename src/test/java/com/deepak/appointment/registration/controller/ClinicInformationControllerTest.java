...
class ClinicInformationControllerTest {
    ...
    @Autowired
    private ObjectMapper objectMapper; // For converting objects to JSON strings if needed for request bodies (not for GET)
    ...
    @MockBean
    private SlotInformationService slotInformationService;
    ...
    void setUp() {
        ...
        clinic.setServicesOffered("General Checkup, Vaccinations");
    ...
    void getAllClinics_shouldReturnListOfClinics() throws Exception {
        ...
        mockMvc.perform(get("/v1/api/get-clinic"))
    ...
    void getClinicById_shouldReturnClinic_whenClinicExists() throws Exception {
        ...
        mockMvc.perform(get("/v1/api/get-clinic/1"))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
    ...
    void getClinicById_shouldReturnNotFound_whenClinicDoesNotExist() throws Exception {
        when(clinicInformationService.getClinicById(1)).thenThrow(new RuntimeException("Clinic not found with id: 1"));
    ...
    void getBasicClinicInfo_shouldReturnBasicInfo() throws Exception {
        when(clinicInformationService.getBasicClinicInfo()).thenReturn(Collections.singletonList(clinicInfoDropDown));

        mockMvc.perform(get("/v1/api/get-clinic-basic"))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON))
            .andExpect(jsonPath("$", hasSize(1)))
    ...
    void getDoctorsForClinic_shouldReturnDoctorList() throws Exception {
        when(doctorInformationService.getDoctorsForClinic(1)).thenReturn(Collections.singletonList(doctorInfoDropDown));

        mockMvc.perform(get("/v1/api/get-clinic/1/doctors"))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON))
            .andExpect(jsonPath("$", hasSize(1)))
    ...
    void getAvailableDates_shouldReturnDatesList() throws Exception {
        ...
        when(slotInformationService.getAvailableDates(1, "doc1")).thenReturn(Collections.singletonList(availableDate));

        mockMvc.perform(get("/v1/api/clinics/1/doctors/doc1/available-dates"))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON))
            .andExpect(jsonPath("$", hasSize(1)))
    ...
    void getAvailableSlots_shouldReturnSlotsMap() throws Exception {
        ...
        AvailableSlotsResponse expectedResponse = new AvailableSlotsResponse(1, "doc1", testDate, slotsData);

        when(slotInformationService.getAvailableSlots(1, "doc1", testDate)).thenReturn(slotsData);

        mockMvc.perform(get("/v1/api/clinics/1/doctors/doc1/slots")
    ...
    void getAvailableSlots_shouldReturnEmptySlotsMap_whenNoSlots() throws Exception {
        ...
        AvailableSlotsResponse expectedResponse = new AvailableSlotsResponse(1, "doc1", testDate, emptySlotsData);
