...
class PatientControllerTest {
    ...
    void createPatient_shouldReturnCreatedPatient_OnSuccess() throws Exception {
        ...
        mockMvc.perform(post("/v1/api/patients")
                .with(csrf())
    ...
    @Test
    void createPatient_shouldReturnInternalServerError_WhenServiceThrowsIllegalArgumentException() throws Exception {
        when(patientService.createPatient(any(Patient.class))).thenThrow(new IllegalArgumentException("Invalid patient data"));

        mockMvc.perform(post("/v1/api/patients")
                .with(csrf())
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(new Patient()))) // Send some data
                .andExpect(status().isInternalServerError()) // Default mapping for unhandled RuntimeExceptions
    ...
    void getPatientByPhoneNumber_shouldReturnPatient_WhenFound() throws Exception {
        ...
        mockMvc.perform(get("/v1/api/patients/by-phone")
                .param("phoneNumber", "+911234567890"))
    ...
    void existsByPhoneNumber_shouldReturnTrue_WhenPatientExists() throws Exception {
        ...
        mockMvc.perform(get("/v1/api/patients/exists-by-phone").param("phoneNumber", "+911234567890"))
            .andExpect(status().isOk())
    ...
    void existsByPhoneNumber_shouldReturnFalse_WhenPatientDoesNotExist() throws Exception {
        ...
        mockMvc.perform(get("/v1/api/patients/exists-by-phone").param("phoneNumber", "unknown"))
            .andExpect(status().isOk())
    ...
    void getPatientById_shouldReturnPatient_WhenIdMatchesAuthenticatedUser() throws Exception {
        ...
        mockMvc.perform(get("/v1/api/patients/by-id").param("id", "1"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id", is(1)));
    }
    ...
    void getPatientById_shouldReturnForbidden_WhenIdDoesNotMatchAuthenticatedUser() throws Exception {
        ...
        mockMvc.perform(get("/v1/api/patients/by-id").param("id", "1")) // Requesting patient 1 as user 2
    ...
    void updatePatient_shouldUpdateAndReturnPatient_WhenAuthorized() throws Exception {
        ...
        mockMvc.perform(put("/v1/api/patients/1")
                .with(csrf())
                .contentType(MediaType.APPLICATION_JSON)
    ...
    void updatePatient_shouldReturnForbidden_WhenNotAuthorized() throws Exception {
        ...
        mockMvc.perform(put("/v1/api/patients/1") // Trying to update patient 1 as user 2
                .with(csrf())
                .contentType(MediaType.APPLICATION_JSON)
    ...
    void updatePatient_shouldReturnNotFound_WhenPatientMissingAndAuthorized() throws Exception {
        ...
        when(patientService.updatePatient(eq(1L), any(Patient.class))).thenReturn(null); // Service indicates not found
    ...
    void deletePatientById_shouldReturnNoContent_WhenAuthorizedAndSuccessful() throws Exception {
        ...
        mockMvc.perform(delete("/v1/api/patients/1").with(csrf()))
    ...
    void deletePatientById_shouldReturnForbidden_WhenNotAuthorized() throws Exception {
        mockMvc.perform(delete("/v1/api/patients/1").with(csrf())) // User 2 trying to delete patient 1
                .andExpect(status().isForbidden());
    }
    ...
    void deletePatientById_shouldReturnNotFound_WhenPatientMissingAndAuthorized() throws Exception {
        ...
        doThrow(new RuntimeException("Patient not found with id: 1")).when(patientService).deletePatient(1L);
        mockMvc.perform(delete("/v1/api/patients/1").with(csrf()))
            .andExpect(status().isNotFound())
    ...
    void updatePassword_shouldReturnOk_WhenAuthorizedAndSuccessful() throws Exception {
        ...
        mockMvc.perform(post("/v1/api/patients/1/password")
                .with(csrf())
                .contentType(MediaType.APPLICATION_JSON)
    ...
    void updatePassword_shouldReturnForbidden_WhenNotAuthorized() throws Exception {
        UpdatePasswordRequest request = new UpdatePasswordRequest();
        request.setNewPassword("newPass123");
        mockMvc.perform(post("/v1/api/patients/1/password") // User 2 trying for patient 1
                .with(csrf())
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request)))
                .andExpect(status().isForbidden());
    ...
    void updatePassword_shouldReturnNotFound_WhenPatientNotFound() throws Exception {
        UpdatePasswordRequest request = new UpdatePasswordRequest();
        request.setNewPassword("newPass123");
        doThrow(new RuntimeException("Patient not found with id: 1")).when(patientService).updatePassword(1L, "newPass123");
