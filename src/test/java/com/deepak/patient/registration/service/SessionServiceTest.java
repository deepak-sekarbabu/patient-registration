...
class SessionServiceTest {
  ...
  @Test
  void isRefreshTokenBlacklisted_shouldReturnFalseAndRemove_forExpiredBlacklistedToken()
    ...
    assertTrue(
        sessionService.isRefreshTokenBlacklisted(token), "Token should be blacklisted initially");

    Thread.sleep(shortExpiryTimeMs + 30); // Wait a bit longer than expiry

    assertFalse(
        sessionService.isRefreshTokenBlacklisted(token),
        "Token should no longer be blacklisted after expiry");
  ...
  @Test
  void blacklistRefreshToken_shouldCleanupOtherExpiredTokensWhenAddingNew()
      throws InterruptedException {
    ReflectionTestUtils.setField(sessionService, defaultRefreshTokenKey, shortExpiryTimeMs);
  ...
  @Test
  void getSessionData_shouldReturnNullAndRemove_forExpiredSessionData()
      throws InterruptedException {
    ...
    assertFalse(
        sessionMap.containsKey(userId), "Expired session data should be removed from the map");
  }

  @Test
  void storeSessionData_shouldCleanupOtherExpiredSessionsWhenAddingNew()
      throws InterruptedException {
    ReflectionTestUtils.setField(sessionService, defaultSessionDataKey, shortExpiryTimeMs);
    ...
    Map<String, ?> sessionMap = getInternalSessionMap(sessionService);
    assertFalse(
        sessionMap.containsKey(userToExpire),
        "Expired session for userToExpire should be cleaned up.");
  ...
  void invalidateSession_shouldRemoveSessionData() {
    ...
    assertNotNull(
        sessionService.getSessionData(userId),
        "Session data should be present before invalidation");

    sessionService.invalidateSession(userId);
    assertNull(
        sessionService.getSessionData(userId), "Session data should be null after invalidation");
    Map<String, ?> sessionMap = getInternalSessionMap(sessionService);
    assertFalse(
        sessionMap.containsKey(userId),
        "Session data should be removed from the map after invalidation");
  }
  ...
  private ConcurrentHashMap<String, Long> getInternalBlacklistMap(SessionService service) {
    return (ConcurrentHashMap<String, Long>)
        ReflectionTestUtils.getField(service, "refreshTokenBlacklist");
  }
